...
solve regex = (rmap, findFar 1000 rmap)
  where
    rmap = generateMap regex


findFar :: Int -> RMap -> [(Coord, Int)]
findFar limit rmap = filter ((>=limit) . snd) $ M.assocs distMap
  where
...
showD (rmap, ans) = unlines [showMap rmap, show (length ans)]
