...
showCaves cs = unlines . map showCave $ zip cs [0..]
...
damage :: Actor -> CaveSquare
damage (Actor otype !hp id)
  | hp <= 3   = CaveF Open
  | otherwise = CaveA (Actor otype (hp-3) id)


takeTurn :: Cave -> (Coord, Actor) -> Cave
takeTurn cave (coord, actor)
  | alreadyDead     = cave
  | otherwise       = cave''
  where
    ...
    --attack
    cave'' = attackInCave actor cave' coord'
...
attackInCave :: Actor -> Cave -> Coord -> Cave
attackInCave actor cave coord
  | (tc, opp):_ <- sortedTargets = cave A.// [(tc, damage opp)]
  | otherwise                    = cave
  where
...
takeRound :: Cave -> (Cave, Bool)
takeRound cave = (cave'', someoneWon cave')
  where
    ...
    cave' = L.foldl' takeTurn cave firstActors
    cave'' = takeTurn cave' lastActor
...
--solve :: Cave -> (Cave, Int)
solve cave = cave : loop cave
  where
    loop c
      | didEnd = [c']
      | otherwise = c':loop c'
      where
        (c', didEnd) = takeRound c
