...
showData :: (Guard, Int) -> String
showData (Guard gid, minute) = unlines $ [resString, sumString]
  where
    resString = show (gid, minute)
...
solve :: [(Timestamp, LogType)] -> (Guard, Int)
solve loglist = (mostSleepyGuard, mostSleptMinute mostSleepyGuard)
  where
    ...
    totalSleeptime g = length $ minutesAsleep g
    mostSleptMinute g = mostCommon $ minutesAsleep g

    guards = List.nub . map fst $ groupedLogs

    mostSleepyGuard = List.maximumBy (Ord.comparing totalSleeptime) guards
