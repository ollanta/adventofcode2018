...
eqri a b r = if readReg a r == b then 1 else 0

eqrr a b r = if readReg a r == readReg b r then 1 else 0


operations = map funcToOperation [addr, addi, mulr, muli, banr, bani, borr, bori ,setr, seti, gtir, gtii, gtrr, eqir, eqri, eqrr]

namedOperations = M.fromList $ zip ["addr", "addi", "mulr", "muli", "banr", "bani", "borr", "bori", "setr", "seti", "gtir", "gtii", "gtrr", "eqir", "eqri", "eqrr"] operations
...
solve (ipointer, program) = helper initregisters
  where
    ...
    -- optimize the inner loop, input-specific
    helper regs@(r0,r1,r2,r3,r4,17) = regs : helper regs'
      where
        regs' = (r0, r1, r2', r3', r4, 26)
        r3' = r4 `quot` 256
        r2' = (r3'+1)*256
    helper regs
      | instr < minI = [regs]
...
showD :: [Registers] -> String
showD = unlines . map show . nondupes S.empty . filter checksEndCondition
  where
    checksEndCondition (_,_,_,_,_,28) = True
    checksEndCondition _ = False

    nondupes :: S.Set Int -> [Registers] -> [Registers]
    nondupes s (x:xs)
      | S.member r1 s = []
      | otherwise  = x : nondupes (S.insert r1 s) xs
      where 
        (_,r1,_,_,_,_) = x
    nondupes _ [] = []
